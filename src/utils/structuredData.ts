...
export function generateStructuredData(options: StructuredDataOptions) {
  ...
  schemas.push({
    ...
    description: "Software engineer and writer exploring fatherhood, masculinity, and modern life through raw, unfiltered reflection.",
    alumniOf: {
      "@type": "Organization",
    ...
    for (const match of matches) {
      if (match[1] && match[2]) {
        ...
        if (question.length > 10 && question.length < 200 && 
            answer.length > 20 && answer.length < 1000) {
...
export function generateReviewSchema(options: {
  ...
  const { name, description, rating, bestRating = 5, worstRating = 1, author, reviewBody, itemReviewed } = options;
...
export function generateEnhancedStructuredData(options: StructuredDataOptions & { content?: string }) {
...
export function validateStructuredData(schema: any): { isValid: boolean; errors: string[]; warnings: string[] } {
  const errors: string[] = [];
