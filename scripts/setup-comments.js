...
const needsUpdate =
  configContent.includes('YOUR_REPO_ID') || configContent.includes('your-username/blog');

if (!needsUpdate) {
  console.log('✅ Giscus configuration appears to be already set up!');
  console.log('If you need to make changes, edit src/config/giscus.ts manually.\n');
  process.exit(0);
}
...
try {
  ...
  const repo = await question('GitHub repository (format: username/repository): ');
  const repoId = await question('Repository ID: ');
  const category = (await question('Category name (default: Comments): ')) || 'Comments';
